...
public class LeonInventoryService {
    ...
    public LeonInventoryService(ItemService itemService, WeaponService weaponService, RecoveryService recoveryService, KeyService keyService) {
    ...
    private void initializeInventory() {
        if (inventory.isEmpty()) {
            inventory.add(itemService.findById(30L).orElse(null));
            inventory.add(itemService.findById(2L).orElse(null));
            inventory.add(itemService.findById(1L).orElse(null));
        }
    }
    ...
    public void darkRoom(Long filmId) {
        KeyItem film = (KeyItem) inventory.stream()
                .filter(i -> i instanceof KeyItem && i.getId().equals(filmId))
                .findFirst().
                orElse(null);
